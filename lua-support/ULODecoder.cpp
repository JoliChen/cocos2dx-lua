...
#include "customtools/ULODecoder.h"
...
{
    bool ret;
    ULODecoder *der = new (std::nothrow) ULODecoder(buffer, len);
    if (der) {
...
ULODecoder::ULODecoder(byte* buffer, const ulo_size &len)
:_buffer(buffer)
,_len(len)
,_pos(0)
,_strlen(1024)
{
    _strbuf = (byte*)malloc(_strlen);
}
...
byte* ULODecoder::getStrBuf(ulo_size size)
{
    if (size > _strlen) {
        ...
        free(_strbuf);
        _strbuf = (byte*)malloc(_strlen);
        //_strbuf = (byte*)realloc(_strbuf, _strlen);
    }
    return _strbuf;
}
